...
T["japanese"] = {
    ...
    "UVC_StatusRestored": "出荷時の一覧に戻しました。トゥイーンメニューで開ける画面のキーは再びキーなしになり、それ以外の操作にはキーが戻りました。保存済み。",
    "UVC_HelpRestore": "全ての操作にキーを戻してから、出荷時の一覧だけをキーなしにします（トゥイーンメニューで開けるジャーナル・所持品・魔法・マップ・スキル・待機のキーボードキー）。そして保存します。",
...
T["korean"] = {
    ...
    "UVC_StatusRestored": "기본 목록으로 돌아갔습니다. 트윈 메뉴가 이미 여는 화면의 키는 다시 해제되고, 나머지 조작은 키를 되찾았습니다. 저장됨.",
    "UVC_HelpRestore": "모든 조작에 키를 되돌린 뒤 기본 목록만 해제합니다(트윈 메뉴가 이미 여는 일지, 소지품, 마법, 지도, 기술, 대기의 키보드 키). 그리고 저장합니다.",
...
T["chinese"] = {
    ...
    "UVC_RestoreBtn": "恢复默认",
    "UVC_StatusRestored": "已恢复为默认列表：Tween 菜单已能打开的界面的按键再次解除，其他操作已恢复按键。已保存。",
    "UVC_HelpRestore": "先恢复所有操作的按键，再只解除默认列表（Tween 菜单已能打开的日志、物品栏、魔法、地图、技能和等待的键盘按键），然后保存。",
...
T["russian"] = {
    ...
    "UVC_StatusRestored": "Возвращён исходный список: клавиши экранов, которые уже открывает меню Tween, снова сняты, остальные действия получили клавиши обратно. Сохранено.",
    "UVC_HelpRestore": "Возвращает клавиши всем действиям, затем снимает только исходный список (клавиши журнала, инвентаря, магии, карты, навыков и ожидания, которые уже открывает меню Tween) и сохраняет.",
...
T["german"] = {
    ...
    "UVC_StatusRestored": "Zurück zur mitgelieferten Liste: Die Tasten für Bildschirme, die das Tween-Menü schon öffnet, sind wieder gelöst, alle anderen Steuerungen haben ihre Taste zurück. Gespeichert.",
    "UVC_HelpRestore": "Gibt jeder Steuerung ihre Taste zurück, löst dann nur die mitgelieferte Liste (die Tastaturtasten für Tagebuch, Inventar, Magie, Karte, Fertigkeiten und Warten, die das Tween-Menü schon öffnet) und speichert.",
...
T["french"] = {
    ...
    "UVC_StatusRestored": "Retour à la liste d'origine : les touches des écrans que le menu Tween ouvre déjà sont de nouveau déliées, toutes les autres commandes ont retrouvé leur touche. Enregistré.",
    "UVC_HelpRestore": "Rend sa touche à chaque commande, puis ne délie que la liste d'origine (les touches clavier du journal, de l'inventaire, de la magie, de la carte, des compétences et de l'attente, que le menu Tween ouvre déjà), et enregistre.",
...
T["spanish"] = {
    ...
    "UVC_StatusRestored": "Vuelta a la lista de serie: las teclas de las pantallas que el menú Tween ya abre vuelven a quedar sin tecla y los demás controles recuperaron la suya. Guardado.",
    "UVC_HelpRestore": "Devuelve su tecla a cada control, luego desvincula solo la lista de serie (las teclas de teclado de diario, inventario, magia, mapa, habilidades y esperar, que el menú Tween ya abre) y guarda.",
...
T["italian"] = {
    ...
    "UVC_StatusRestored": "Tornato all'elenco predefinito: i tasti delle schermate che il menu Tween apre già sono di nuovo slegati, tutti gli altri comandi hanno riavuto il loro tasto. Salvato.",
    "UVC_HelpRestore": "Restituisce il tasto a ogni comando, poi slega solo l'elenco predefinito (i tasti da tastiera di diario, inventario, magia, mappa, abilità e attesa, che il menu Tween apre già) e salva.",
...
T["polish"] = {
    ...
    "UVC_StatusRestored": "Przywrócono domyślną listę: klawisze ekranów, które menu Tween już otwiera, znów są odpięte, a pozostałe sterowania odzyskały klawisze. Zapisano.",
    "UVC_HelpRestore": "Przywraca klawisz każdemu sterowaniu, potem odpina tylko domyślną listę (klawisze dziennika, ekwipunku, magii, mapy, umiejętności i czekania, które menu Tween już otwiera) i zapisuje.",
...
T["czech"] = {
    ...
    "UVC_StatusRestored": "Zpět na výchozí seznam: klávesy obrazovek, které už otevírá nabídka Tween, jsou opět odpojené a ostatní ovládání má svou klávesu zpět. Uloženo.",
    "UVC_HelpRestore": "Vrátí klávesu každému ovládání, pak odpojí jen výchozí seznam (klávesy deníku, inventáře, magie, mapy, dovedností a čekání, které už otevírá nabídka Tween) a uloží.",
